...
import (
	...
	"gorm.io/gorm"
	"management_student1/controllers"
)
...
func SetupRouter(db *gorm.DB) *gin.Engine {
	...
	studentGroup := r.Group("/students")
	{
		...
		studentGroup.GET("/search", func(c *gin.Context) { controllers.SearchStudent(c, db) })
	}
	subjectGroup := r.Group("/subject")
	{
		subjectGroup.POST("/", TeacherOnly, func(c *gin.Context) { controllers.AddSubject(c, db) })
		subjectGroup.POST("/assign", TeacherOnly, func(c *gin.Context) { controllers.AssginSubjectToStudent(c, db) })
	}
